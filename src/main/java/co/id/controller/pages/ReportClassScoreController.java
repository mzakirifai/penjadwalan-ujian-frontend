...
public class ReportClassScoreController {
    ...
    private void onViewReport() {
        ...
        if (selectedClassroom == null || examType == null || semester == null || academicYear == null || academicYear.isBlank()) {
            new Alert(Alert.AlertType.WARNING, "Kelas, Jenis Ujian, Semester, dan Tahun Akademik wajib diisi!", ButtonType.OK).showAndWait();
            return;
        }

        try {
            JasperReport jasperReport = (JasperReport) JRLoader.loadObject(
                getClass().getResourceAsStream("/reports/ReportClassScore.jasper")
            );

            List<ClassScoreRecapItem> data = reportService.getClassScoreRecapReport(
                selectedClassroom.getId(), examType, semester, academicYear
            );

            if (data.isEmpty()) {
                new Alert(Alert.AlertType.INFORMATION, "Belum ada nilai untuk kelas & periode ini.", ButtonType.OK).showAndWait();
                return;
            }

            // ambil detail kelas lengkap (dengan Major), karena getAllClassrooms belum JOIN sampai Jurusan
