...
public class ReportTeacherScheduleController {
    ...
    public void initialize() {
        ...
        TableColumn<Teacher, String> colNip = new TableColumn<>("NIP");
        colNip.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getNip()));

        TableColumn<Teacher, String> colName = new TableColumn<>("Nama");
        colName.setCellValueFactory(d -> new SimpleStringProperty(d.getValue().getName()));

        lookupBoxTeacher.configure(
            () -> masterService.getAllTeachers(), List.of(colNip, colName), Teacher::getName
        );
    ...
    private void onViewReport() {
        Teacher selectedTeacher = lookupBoxTeacher.getSelectedItem();
        ...
        if (selectedTeacher == null || examType == null || semester == null || academicYear == null || academicYear.isBlank()) {
            new Alert(Alert.AlertType.WARNING, "Guru, Jenis Ujian, Semester, dan Tahun Akademik wajib diisi!", ButtonType.OK).showAndWait();
            return;
        }

        try {
            JasperReport jasperReport = (JasperReport) JRLoader.loadObject(
                getClass().getResourceAsStream("/reports/ReportTeacherSchedule.jasper")
            );

            List<ExamScheduleReportItem> data = reportService.getTeacherScheduleReport(
                selectedTeacher.getId(), examType, semester, academicYear
            );

            if (data.isEmpty()) {
                new Alert(Alert.AlertType.INFORMATION, "Guru ini tidak mengawas sesi ujian manapun pada periode ini.", ButtonType.OK).showAndWait();
                return;
            }

            Map<String, Object> parameters = new HashMap<>();
            parameters.put("TEACHER_NAME", selectedTeacher.getName());
            parameters.put("TEACHER_NIP", selectedTeacher.getNip());
